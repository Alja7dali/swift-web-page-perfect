...
let advancedUsage = {
  return div {
    ...
    """)
    p {
      text("Now, in your router you can wrap the content that is specific to that page in your layout by just invoking the ")
      code("layout(content:)")
      text(" function:")
    }
    pre("""
    ...
    router.add(method: .get, uri: "/about") { request, response in
      response.append(
        layout(
